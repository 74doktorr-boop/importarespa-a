...
import { X, Save, Settings, DollarSign, FileText, Truck, ShieldCheck, Calculator, Download, Eye, EyeOff } from 'lucide-react';
import { generateVehicleReportV2 } from '../utils/pdfGenerator';

const AdminQuoteModal = ({ isOpen, onClose, vehicleData, transportCost }) => {
    ...
    }, [transportCost, settings.transportOverride]);
    ...
    const handleGenerateQuote = () => {
        ...
        // We will call a specific function in pdfGenerator (to be implemented)
        // For now, we reuse the existing one but we'll modify it next
            ...
            {isOpen && (
                <>
                    ...
                    <motion.div
                        ...
                        className="fixed inset-0 m-auto max-w-4xl h-fit max-h-[90vh] bg-white rounded-2xl shadow-2xl z-[70] overflow-hidden flex flex-col"
                    >
                        {/* Header */}
                        ...
                        <div className="flex-1 overflow-y-auto p-6">
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                                ...
                                <div className="space-y-6">
                                    ...
                                    <div className="bg-slate-50 p-5 rounded-xl border border-slate-200">
                                        <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wider mb-4 flex items-center gap-2">
                                            <Truck size={16} className="text-blue-600" /> Transporte
                                        </h3>
                                        <div className="flex gap-4 items-center">
                                            <div className="flex-1">
                                                <label className="block text-xs font-medium text-slate-500 mb-1">Calculado (Auto)</label>
                                                <div className="px-3 py-2 bg-slate-200 text-slate-500 rounded-lg font-mono text-sm">
                                                    {transportCost} €
                                                </div>
                                            </div>
                                            <div className="flex-1">
                                                <label className="block text-xs font-medium text-slate-900 mb-1">Manual (Override)</label>
                                                <input
                                                    type="number"
                                ...
                                {/* Right: Real-time Quote Preview */}
                                <div className="flex flex-col h-full">
                                    <div className="bg-white border-2 border-slate-900 rounded-xl p-6 shadow-lg flex-1 flex flex-col">
                                        <div className="flex justify-between items-center mb-6">
                                            <h3 className="text-lg font-bold text-slate-900">Vista Previa Presupuesto</h3>
