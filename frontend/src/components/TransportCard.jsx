    ...
    return (
        <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-3xl p-6 shadow-2xl relative overflow-hidden group hover:bg-white/10 transition-all duration-300">
            <div className="flex items-center mb-6 relative z-10">
                <div className="p-3 bg-blue-500/20 rounded-xl mr-4 text-blue-400">
                    <Truck size={24} />
                </div>
                <div>
            ...
            {!result ? (
                <form onSubmit={handleCalculate} className="relative z-10">
                    <div className="space-y-5">
                        ...
                        <div className="flex items-center justify-between px-2 relative">
                            <div className="absolute left-4 right-4 top-1/2 h-0.5 bg-white/10 -z-10"></div>

                            <div className="flex flex-col items-center bg-white/10 p-2 rounded-lg border border-white/5 shadow-sm">
                                <MapPin size={16} className="text-slate-400 mb-1" />
                                <span className="text-[10px] font-bold text-slate-300 uppercase">{originCity ? originCity.substring(0, 3) : 'DE'}</span>
                            </div>
                            ...
                            <div className="flex flex-col items-center bg-white/10 p-2 rounded-lg border border-white/5 shadow-sm">
                                ...
                                <span className="text-[10px] font-bold text-blue-400 uppercase">ES</span>
                            </div>
                        </div>
                        ...
                        <div className="relative group/input">
                            ...
                            <input
                                ...
                                placeholder="28001"
                                maxLength={5}
                                className="w-full bg-white/5 border border-white/10 rounded-xl py-4 pl-24 pr-12 text-white placeholder-slate-500 focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 transition-all outline-none font-mono text-lg tracking-widest"
                                value={zipCode}
                                onChange={(e) => {
                            ...
                            <button
                                ...
                                className="absolute right-2 top-2 bottom-2 bg-blue-600 hover:bg-blue-500 text-white rounded-lg px-4 flex items-center justify-center transition-all shadow-lg hover:shadow-blue-900/20 disabled:opacity-50 disabled:cursor-not-allowed disabled:shadow-none"
                            >
                                {loading ? <Loader2 size={20} className="animate-spin" /> : <ArrowRight size={20} />}
                            </button>
                        </div>

                        {error && <p className="text-red-400 text-xs mt-2 ml-1 flex items-center"><span className="w-1.5 h-1.5 bg-red-400 rounded-full mr-2"></span>{error}</p>}
                    </div >
                </form >
            ) : (
                <motion.div
                ...
                >
                    <div className="bg-black/40 rounded-2xl p-6 shadow-inner mb-5 relative overflow-hidden text-center border border-white/5">
                    ...
                    <button
                        ...
                        className="w-full py-3 rounded-xl border border-white/10 text-xs font-bold text-slate-400 hover:text-white hover:bg-white/10 transition-all uppercase tracking-wider"
                    >
                        Recalcular Ruta
                    </button>
                </motion.div>
