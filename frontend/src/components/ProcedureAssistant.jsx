...
import { CheckCircle, Circle, Clock, FileText, Landmark, ShieldCheck, CreditCard, ClipboardList, Info, ChevronDown, ExternalLink } from 'lucide-react';
...
const ProcedureAssistant = () => {
    const [steps, setSteps] = useState(() => {
        ...
        if (saved) {
            try {
                ...
                // Si encontramos iconos en el objeto guardado (antigua versión), los limpiamos
                // para evitar que se guarden como objetos vacíos {}
                return parsed.map(step => ({
                    ...step,
                    // Si el objeto guardado tiene 'icon' y es un objeto vacío (bloqueado por JSON.stringify),
                    // o si no tiene 'iconName', le asignamos uno por defecto o el que corresponda
                    iconName: step.iconName || (step.id === 1 ? 'FileText' : step.id === 2 ? 'ClipboardList' : step.id === 3 ? 'ShieldCheck' : step.id === 4 ? 'CreditCard' : step.id === 5 ? 'Landmark' : 'CheckCircle')
                }));
            } catch (e) {
        ...
        return [
            { id: 1, title: 'Documentación de Origen', desc: 'Asegúrate de tener el Permiso de Circulación (Teil I y II) y el COC (Certificado de Conformidad).', iconName: 'FileText', done: false },
            { id: 2, title: 'Ficha Técnica Reducida', desc: 'Contrata un ingeniero para que emita la ficha técnica si el COC no es suficiente.', iconName: 'ClipboardList', done: false },
            { id: 3, title: 'ITV de Importación', desc: 'Pide cita en una estación ITV para la inspección extraordinaria de importación.', iconName: 'ShieldCheck', done: false },
            { id: 4, title: 'Modelo 576 (Hacienda)', desc: 'Liquidación del Impuesto de Matriculación ante la AEAT basado en emisiones CO2.', iconName: 'CreditCard', done: false, link: 'https://sede.agenciatributaria.gob.es/' },
            { id: 5, title: 'Impuesto de Tracción Mecánica', desc: 'Pago del impuesto de circulación (numerito) en tu ayuntamiento.', iconName: 'Landmark', done: false },
            { id: 6, title: 'Matriculación Final (DGT)', desc: 'Solicita el número de matrícula y el permiso definitivo en Tráfico.', iconName: 'CheckCircle', done: false, link: 'https://sede.dgt.gob.es/' }
        ];
    });
    ...
    useEffect(() => {
        // Al guardar, guardamos el estado actual que ya usa iconName (strings)
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-8 shadow-sm relative overflow-hidden group">
            ...
            <div className="grid grid-cols-1 gap-3 relative">
                ...
                {steps.map((step, index) => {
                    const StepIcon = ICON_MAP[step.iconName] || Info;
                    return (
                        <div
                            key={step.id}
                            ...
                            >
                                <div className="flex items-center gap-4">
                                    ...
                                    <div className="flex items-center gap-3">
                                        <div className={step.done ? 'text-blue-600 dark:text-blue-400' : 'text-slate-400'}>
                                            <StepIcon size={20} />
                                        </div>
